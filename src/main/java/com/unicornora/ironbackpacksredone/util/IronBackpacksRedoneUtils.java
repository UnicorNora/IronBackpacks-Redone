...
import com.unicornora.ironbackpacksredone.item.upgrade.UpgradeHotbarSwapper;
...
public class IronBackpacksRedoneUtils
{
    ...
    public static Method getMethodSafe(Class<?> clazz, Class[] params, String... names)
    {
        ...
        {
            FMLCommonHandler.instance().raiseException(e, "IronBackpacksRedone was unable to reflect a constructor!", true);
        }
    ...
    public static Class<?> getOptionalClass(String clazzName, BooleanSupplier optionalTester)
    {
        if (optionalTester == null || optionalTester.getAsBoolean())
        {
            ...
            {
                FMLCommonHandler.instance().raiseException(ex, "IronBackpacksRedone couldn't reflect a class. This is probably due to compatibility patches not finding the mod required", false);
                return null;
            }
        }
    ...
    public static Field getFieldSafe(Class<?> clazz, String... fieldNames)
    {
        ...
        {
            FMLCommonHandler.instance().raiseException(ex, "IronBackpacksRedone was unable to reflect field!", true);
        }
    ...
    public static ItemStack getBackpack(EntityPlayer player, int slot)
    {
        if (slot == -1)
        {
            return IronBackpacksRedonePlayer.of(player).getCurrentBackpack();
        }
        else
        {
