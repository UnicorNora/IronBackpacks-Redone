...
{
    private static final Lazy<Class<?>> classEnderStorageManager = new Lazy<>(() -> IronBackpacksRedoneUtils.getOptionalClass("codechicken.enderstorage.manager.EnderStorageManager", () -> Loader.isModLoaded("enderstorage")));
    private static final Lazy<Class<?>> classFrequency = new Lazy<>(() -> IronBackpacksRedoneUtils.getOptionalClass("codechicken.enderstorage.api.Frequency", () -> Loader.isModLoaded("enderstorage")));
    private static final Lazy<Class<?>> classChestHelper = new Lazy<>(() -> IronBackpacksRedoneUtils.getOptionalClass("shetiphian.multistorage.common.enderlink.ChestHelper", () -> Loader.isModLoaded("multistorage")));
    private static final Lazy<Method> frequency_fromString = new Lazy<>(() -> IronBackpacksRedoneUtils.getMethodSafe(classFrequency.get(), new Class[]{ String.class, String.class, String.class }, "fromString"));
    private static final Lazy<Method> enderStorageManager_instance = new Lazy<>(() -> IronBackpacksRedoneUtils.getMethodSafe(classEnderStorageManager.get(), new Class[]{ boolean.class }, "instance"));
    private static final Lazy<Method> enderStorageManager_getStorage = new Lazy<>(() -> IronBackpacksRedoneUtils.getMethodSafe(classEnderStorageManager.get(), new Class[]{ classFrequency.get(), String.class }, "getStorage"));
    private static final Lazy<Method> chestHelper_getChest = new Lazy<>(() -> IronBackpacksRedoneUtils.getMethodSafe(classChestHelper.get(), new Class[]{ World.class, String.class, String.class }, "getChest"));

    public UpgradeEnderStorage()
    {
        super(IronBackpacksRedoneRegistryNames.itemUpgradeEnderStorage);
    ...
    private String dyeToColour(ItemStack dye)
    {
        if (!dye.isEmpty())
        {
            for (String s : IronBackpacksRedoneUtils.getOreNames(dye))
            {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
        ...
        tooltip.addAll(Arrays.asList(I18n.format("ironbackpacksredone.txt.upgrade.ender_storage.desc").split("\\|")));
    }
        ...
        if (!is.isEmpty() && IBackpack.of(is) == null && (filter == null || filter.accepts(is)))
        {
            ...
            if (!dye0.isEmpty() && !dye1.isEmpty() && !dye2.isEmpty())
            {
                if (!useMultiStorage)
                {
                    ...
                    {
                        Object frequency = IronBackpacksRedoneUtils.invokeMethod(frequency_fromString.get(), null, dyeName0, dyeName1, dyeName2);
                        Object enderStorageManager = IronBackpacksRedoneUtils.invokeMethod(enderStorageManager_instance.get(), null, false);
                        Object storage = IronBackpacksRedoneUtils.invokeMethod(enderStorageManager_getStorage.get(), enderStorageManager, frequency, "item");
                ...
                else
                {
                    ...
                    {
                        Object chest = IronBackpacksRedoneUtils.invokeMethod(chestHelper_getChest.get(), null, ticker.world, "all", dyeCode0 + dyeCode1 + dyeCode2);
    ...
    {
        IronBackpacksRedoneUtils.openContainer(player, new ContainerUpgradeEnderStorage(player.inventory, stack, slot));
        IronBackpacksRedoneNet.sendOpenGUI(player, slotID, inventory, slot, EnumGuiType.UPGRADE_ENDER_STORAGE);
    }
}
