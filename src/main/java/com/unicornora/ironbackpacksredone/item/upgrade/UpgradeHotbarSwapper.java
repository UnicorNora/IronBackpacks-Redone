...
public class UpgradeHotbarSwapper extends ItemSimple implements IUpgrade
{
    ...
    {
        super(IronBackpacksRedoneRegistryNames.itemUpgradeHotbarSwapping, 1);
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
        ...
        tooltip.addAll(Arrays.asList(I18n.format("ironbackpacksredone.txt.upgrade.hotbar.desc").split("\\|")));
    }
    ...
    {
        return backpack.getBackpackType() != EnumBackpackType.DIAMOND && !Arrays.stream(backpack.getReadonlyUpdatesArray()).filter(Objects::nonNull).map(IUpgradeWrapper::getSelf).anyMatch(i -> i.getItem() == self.getSelf().getItem());
    }
