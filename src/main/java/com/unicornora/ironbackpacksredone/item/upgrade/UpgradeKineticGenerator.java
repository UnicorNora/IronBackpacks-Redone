...
public class UpgradeKineticGenerator extends ItemSimple implements IUpgrade
{
    ...
    {
        super(IronBackpacksRedoneRegistryNames.itemUpgradeKineticGenerator, 1);
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
        ...
        tooltip.addAll(Arrays.asList(I18n.format("ironbackpacksredone.txt.upgrade.generator_kinetic.desc").split("\\|")));
    }
    ...
    public void onPulse(@Nullable IBackpackWrapper container, IBackpackWrapper backpack, IUpgradeWrapper self, Entity pulsar)
    {
        ...
        int power = (int) Math.round(new Vector3d(pulsar.posX - prevPosX, pulsar.posY - prevPosY, pulsar.posZ - prevPosZ).length() * IronBackpacksRedoneCfg.kineticUpgradeFEPerMeter);
        if (power > 0)
        {
