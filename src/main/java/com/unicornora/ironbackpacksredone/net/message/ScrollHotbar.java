    ...
    public static class Handler implements IMessageHandler<ScrollHotbar, IMessage>
    {
        ...
        public IMessage onMessage(ScrollHotbar message, MessageContext ctx)
        {
            ...
            if (message.slotID < sender.inventory.getSizeInventory())
            {
                ...
                {
                    ItemStack backpack = IronBackpacksRedoneUtils.checkBackpackForHotbarUpgrade(IronBackpacksRedoneUtils.getBackpack(sender, message.slotID));
                    if (!backpack.isEmpty())
                    {
                        ...
                        if (upgrade != ItemStack.EMPTY)
                        {
                            ...
                            index = index < 0 ? IronBackpacksRedoneUtils.getBackpackRows(iBackpack.createWrapper().getBackpackType()) - 1 : index >= IronBackpacksRedoneUtils.getBackpackRows(iBackpack.createWrapper().getBackpackType()) ? 0 : index;
