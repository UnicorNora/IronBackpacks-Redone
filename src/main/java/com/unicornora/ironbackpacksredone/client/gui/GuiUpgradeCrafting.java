...
public class GuiUpgradeCrafting extends GuiContainer
{
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        ...
        if (button.id < 9)
        {
            ...
            if (craftingUpgrade != null)
            {
                ...
                IronBackpacksRedoneNet.sendChangeOreDictParam(button.id);
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        int i = (this.width - this.xSize) / 2;
        int j = (this.height - this.ySize) / 2;
        Minecraft.getMinecraft().renderEngine.bindTexture(IronBackpacksRedoneCfg.useLightUI ? IronBackpacksRedoneTextures.UPGRADE_CRAFTING_LIGHT : IronBackpacksRedoneTextures.UPGRADE_CRAFTING);
    ...
    private class OreDictButton extends GuiButton
    {
        ...
        public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks)
        {
            if (this.visible)
            {
                ...
                this.drawCenteredString(Minecraft.getMinecraft().fontRenderer, I18n.format("ironbackpacksredone.crafting.oredict." + oreDict), this.x + 3, this.y - 1, -1);
