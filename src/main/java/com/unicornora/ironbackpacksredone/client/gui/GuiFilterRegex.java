...
public class GuiFilterRegex extends GuiContainer
{
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        ...
        if (button.id < 4)
        {
            IronBackpacksRedoneNet.sendChangeFilterParam(button.id);
        }
    }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException
    {
        ...
        if (!IronBackpacksRedoneUtils.areStringsEqual(current, this.input.getText()))
        {
            ...
            IronBackpacksRedoneNet.sendChangeFilterPattern(this.input.getText());
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        int i = (this.width - this.xSize) / 2;
        int j = (this.height - this.ySize) / 2;
        Minecraft.getMinecraft().renderEngine.bindTexture(IronBackpacksRedoneCfg.useLightUI ? IronBackpacksRedoneTextures.FILTER_REGEX_LIGHT : IronBackpacksRedoneTextures.FILTER_REGEX);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (GuiButton btn : this.buttonList)
        {
            if (btn.isMouseOver())
            {
                ...
                this.drawHoveringText(I18n.format("ironbackpacksredone.filter.state." + btn.id + "." + postfix), mouseX, mouseY);
            }
        }
        ...
        {
            this.drawHoveringText(this.regexCorrect ? I18n.format("ironbackpacksredone.filter.regex.ok") : this.lastException, mouseX, mouseY);
        }
    }
    ...
    private class FilterButton extends GuiButton
    {
        ...
        public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks)
        {
            if (this.visible)
            {
                mc.getTextureManager().bindTexture(IronBackpacksRedoneTextures.WIDGETS);
