...
public class FilterRegex implements IFilter
{
    ...
    {
        boolean test = this.regex == null ? !this.whitelist : this.oreDict ? IronBackpacksRedoneUtils.anyMatch(IronBackpacksRedoneUtils.getOreNames(is), s -> this.regex.matcher(s).matches()) : this.regex.matcher(is.getItem().getRegistryName().toString()).matches();
