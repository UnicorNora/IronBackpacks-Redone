package com.unicornora.ironbackpacksredone.capability;
...
public class Backpack implements IBackpack
{
    ...
    private class Wrapper implements IBackpackWrapper
    {
        ...
        {
            return IronBackpacksRedoneUtils.capabilityToArray(Backpack.this.upgrades, IUpgradeWrapper.class, i -> i.getItem() instanceof IUpgrade ? new UpgradeWrapper(i) : null);
        }

        @Override
        public ItemStack[] getReadonlyInventory()
        {
            return IronBackpacksRedoneUtils.capabilityToArray(Backpack.this.inventory, ItemStack.class, i -> i);
        }
    ...
    private class NBTItemHandler implements IItemHandler, IItemHandlerModifiable, INBTSerializable<NBTTagCompound>
    {
        ...
        {
            if (!this.stack.hasTagCompound() || !this.stack.getTagCompound().hasKey("ironbackpacksredone:nbtItemHandler", Constants.NBT.TAG_COMPOUND))
            {
                return;
            }
            ...
            NBTTagCompound tag = this.stack.getTagCompound().getCompoundTag("ironbackpacksredone:nbtItemHandler");
            tag.removeTag("Size");
            this.inventory.deserializeNBT(this.stack.getTagCompound().getCompoundTag("ironbackpacksredone:nbtItemHandler"));
        }
        ...
        public void deserializeNBT(NBTTagCompound nbt)
        {
            ...
            {
                if (nbt.hasKey("ironbackpacksredone:nbtItemHandler"))
                {
                    this.inventory.deserializeNBT(nbt.getCompoundTag("ironbackpacksredone:nbtItemHandler"));
                }
                else
                {
    ...
    private class EnergyStorageNBT extends EnergyStorage
    {
        ...
        public int getEnergyStored()
        {
            return stack.hasTagCompound() ? stack.getTagCompound().getInteger("ironbackpacksredone:energy") : 0;
        }
        ...
        void setEnergyStored(int value)
        {
            ...
            stack.getTagCompound().setInteger("ironbackpacksredone:energy", value);
        }
    }
}
