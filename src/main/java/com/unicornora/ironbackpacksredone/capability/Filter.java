...
public class Filter implements IFilter
{
    ...
    public boolean accepts(ItemStack is)
    {
        ...
        for (int i = 0; i < this.inventory.getSlots(); i++)
        {
            ...
            else
            {
                if (this.oreDict)
                {
                    if (IronBackpacksRedoneUtils.isOreDictionaryMatch(is, stack))
                    {
    ...
    private void checkV2()
    {
        ...
        {
            ItemStack[] stacks = IronBackpacksRedoneUtils.capabilityToArray(this.inventory, ItemStack.class, s -> s);
